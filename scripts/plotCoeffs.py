...
if __name__ == '__main__':
    ...
    if d['mode'].startswith('herm'):
        ...
        #ft
        usize=(float(len(ry))/float(len(rx)))*1./np.abs(rx[0]-rx[1]) #!!!
        vsize=(float(len(rx))/float(len(ry)))*1./np.abs(ry[0]-ry[1]) #!!!
        #print usize,vsize
        ures=(float(len(ry))/float(len(rx)))*1./np.abs(rx[0]-rx[-1]) #!!!
        vres=(float(len(rx))/float(len(ry)))*1./np.abs(ry[0]-ry[-1]) #!!!
        #print ures,vres
        umin=ures*(rx[0]+.5)
        vmin=vres*(ry[0]+.5)
        ...
        usize=5.*1./np.abs(rx[0]-rx[1]) #!!!
        vsize=5.*1./np.abs(ry[0]-ry[1]) #!!!
        ures=5.*1./np.abs(rx[0]-rx[-1]) #!!!
        vres=5.*1./np.abs(ry[0]-ry[-1]) #!!!
        uu=np.arange(-.5*usize,.5*usize,ures)
        vv=np.arange(-.5*vsize,.5*vsize,vres)
        #print len(uu),len(vv)
        #print uu,vv
        bvals=[]
        ...
        #uu=np.array(range(0,d['size'][0]),dtype=float)-d['xc'][0]
        #vv=np.array(range(0,d['size'][1]),dtype=float)-d['xc'][1]
        ##print uu
        ##print vv
        #bvals=[]
    ...
    p.subplot(234)
    mdl_fft2=np.fft.fft2(mdl)
    axis0_shift=int(mdl_fft2.shape[0]/2)
    axis1_shift=int(mdl_fft2.shape[1]/2)
    mdl_fft2=np.concatenate((mdl_fft2[axis0_shift:,:],mdl_fft2[:axis0_shift,:]),axis=0)
    mdl_fft2=np.concatenate((mdl_fft2[:,axis1_shift:],mdl_fft2[:,:axis1_shift]),axis=1)
    p.imshow(np.abs(mdl_fft2))
    ...
    p.subplot(235)
    mdl_ft_fft2=np.fft.fft2(mdl_ft)
    axis0_shift=int(mdl_ft_fft2.shape[0]/2)
    axis1_shift=int(mdl_ft_fft2.shape[1]/2)
    mdl_ft_fft2=np.concatenate((mdl_ft_fft2[axis0_shift:,:],mdl_ft_fft2[:axis0_shift,:]),axis=0)
    mdl_ft_fft2=np.concatenate((mdl_ft_fft2[:,axis1_shift:],mdl_ft_fft2[:,:axis1_shift]),axis=1)
    p.imshow(np.abs(mdl_ft_fft2))
